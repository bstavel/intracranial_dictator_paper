...
For the first year and a half of these analyses, I calculated the test statistic for the permutations each time we made developed a new analysis. Then, to create a null distribution on the scatter plots for the clustering analysis visualozation, I calculated an average R2 for each permute. I did some EDA on those null permutations and realized there was an error in my logic: the clear visualization of our average r2 clusters clearly showed a lack of uniform encoding, but that does *not* imply the existence of split encoding (duh). And the null distribution looked very similar because there was also no uniform encoding in the null distribution.

That ended the idea that the clustering was evidence for split encoding, so we returned to needing a test statistic to prove split encoding. The first part of this script compiles the nul distributions. The second part creates the test statistics. The third part is a v messy EDA from the earlier iterations where I was figuring out that the the clustering approach didn't make sense (at least how I was using it at first.)
...
#                                "compiled_null_ttype_results_12_8_21.csv"))
