...
dis_only_ofc_results <- read_csv(
  path(here(), "results", "single_regressions", "dis_only_ofc_theta_ogpermutation_single_regressions_10_10_2022.csv"),
  col_types = cols())
adv_only_ofc_results <- read_csv(
  path(here(), "results", "single_regressions", "adv_only_cing_theta_ogpermutation_single_regressions_10_10_2022.csv"),
  col_types = cols())
...
write_csv(split_ofc_results, path(here(), "results", "single_regressions", "compiled_ofc_theta_ogpermutation_single_regressions_10_15_2022.csv"))
...
adv_files <- filenames[grepl("*null-average-r2-adv-theta", filenames)]
dis_files <- filenames[grepl("*null-average-r2-dis-theta", filenames)]
...
write_csv(complete_null_results, path(here(), "results", "null_average_r2",
                                      "compiled_null_theta_results_ogpermutation_1_28_22.csv"))
```
...
## version with only sig r2
# null_test_stat_df <- complete_null_results_prepped %>%
#   mutate(r2_sig = if_else(p < 0.05, R2, 0)) %>%
#   select(-X1, -Beta, -p, -R2, -elec_ids) %>%
#   mutate(permute_id = paste(subject, electrode, predictor, epoch, permutation, sep = "_")) %>%
#   group_by(permute_id) %>%
#   pivot_wider(names_from = state, values_from = r2_sig) %>%
#   mutate(test_stat = sum(abs(Disadvantageous - Advantageous )))

## beta version ##
# null_test_stat_df <- complete_null_results_prepped %>%
#   select(-X1, -p, -R2, -elec_ids) %>%
#   mutate(permute_id = paste(subject, electrode, predictor, epoch, permutation, sep = "_")) %>%
#   group_by(permute_id) %>%
#   pivot_wider(names_from = state, values_from = Beta) %>%
#   mutate(test_stat = sum(abs(Disadvantageous - Advantageous )))
...
true_ofc_split_results <- read_csv('results/single_regressions/compiled_ofc_theta_ogpermutation_single_regressions_10_15_2022.csv')
...
write_csv(split_true_ofc_tstats_wpvals, 'results/single_regressions/compiled_ofc_theta_ogpermutation_wpvalues_single_regressions_10_15_2022.csv')
```
...
write_csv(cutoff_df, path(here(), "results", "null_average_r2", 
                          "adv_dis_theta_r2_sig_difference_test_stat_10_15_2022.csv"))

write_csv(cutoff_df_full, path(here(), "results", "null_average_r2",
                               "adv_dis_theta_r2_sig_difference_test_stat_top50_10_5_2022.csv"))
...
filenames <- dir(path(here(), "results", "null_average_r2"))
# separate adv and dis #
ttype_files <- filenames[grepl("*ttype-permute-theta-cperm-hilbertRS", filenames)]
...
write_csv(null_ttype_df, path(here(), "results", "null_average_r2",
                              "compiled_null_theta_ttype_results_10_15_22.csv"))
```
...
null_ttype_df <- read_csv(path(here(), "results", "null_average_r2",
                               "compiled_null_theta_ttype_results_10_15_22.csv"))
...
write_csv(split_true_ofc_tstats_wpvals, 'results/single_regressions/compiled_ofc_theta_ogpermutation_ttype_wpvalues_single_regressions_10_15_2022.csv')
```
...
write_csv(cutoff_ttype_df, path(here(), "results", "null_average_r2",
                                "ttype_difference_theta_test_stat_12_8_2021.csv"))

write_csv(cutoff_ttype_full, path(here(), "results", "null_average_r2",
                                "ttype_difference_theta_test_stat_top_50_12_8_2021.csv"))
```
