...
# Inequity Dependent ("split") Analyses (Theta)

This notebook takes the compiled results from the permutated regressions, does FDR correction within the theta band.  Creates the supplementary table 3.


```{r load-data}
# load permuted regressor results #
split_ofc_encoding_reg_df <- read_csv(path(here(), "results", "single_regressions",
                                       "compiled_ofc_theta_ogpermutation_wpvalues_single_regressions_10_15_2022.csv"))

# load permuted trial results #
split_ofc_encoding_ttype_df <- read_csv(path(here(), "results", "single_regressions",
                                       "compiled_ofc_theta_ogpermutation_ttype_wpvalues_single_regressions_10_15_2022.csv"))
...
## table
split_theta_fdr_sig_table <- split_ofc_encoding_df_fdr %>%
  ...
  select(predictor, epoch, sig_count_per_epoch, unique_subs_by_epoch, percent_of_elecs_per_epoch, 
         sig_count, unique_subs, percent_of_elecs) %>%
  distinct() %>%
...
split_ofc_encoding_df_fdr %>%
  ...
  ggtitle("theta Split Encoding")

split_theta_fdr_sig_table %>%
  kable() %>%
  kable_styling()
```
