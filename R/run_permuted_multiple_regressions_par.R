run_permuted_multiple_regressions_par <- function(brain_behave_data, 
                                         electrodes, 
                                         regressor, 
                                         regressor_control,
                                         nBins, 
  ...
  for(elec in electrodes) {
    ...
    colnames(results) <- c("electrode", "bin", "R2", "Beta", "Intercept", "Tstat", "p", 
                           "fstretch", "bstretch", "perm_p", "perm_p_beta", 
                           "beta_null_max", "beta_null_mean", "beta_null_sd",
                           "tstat_null_max", "tstat_null_mean", "tstat_null_sd")
    ...
    tryCatch(
      expr = {
        for (bin in nBins) {      
          ...
          control_vec <- brain_behave_data_elec %>% pull(regressor_control)
          model <- summary(lm(bin_vec ~ reg_vec + control_vec))
          
          # store info from models #
          r2[bin] <- model$r.squared
          tstat[bin] <- model$coefficients[2,3]
    ...
    if( tstat_stretch > 0 ) {
      ...
      null_stretch <- foreach(h = 1:niter, .inorder=FALSE, .combine = 'rbind') %dopar% {
        ...
        for (bin in nBins) { 
          ...
          reg_vec <- brain_behave_data_elec %>% pull(regressor)
          control_vec <- brain_behave_data_elec %>% pull(regressor_control)
          null_lm <-summary(lm(sample(bin_vec) ~ reg_vec + control_vec, data = brain_behave_data_elec))
          
          # save info from models #
          null_tstat[bin] <- null_lm$coefficients[2,3]
        ...
        if(is.na(indices[1])) {
          ...
          null_stretch[h, 1] <- max(abs(null_tstat))
          null_stretch[h, 2] <- max(abs(null_beta))
          
        } else {
          # if a stretch take the sum of f stats #  
          null_stretch[h, 1] <- sum(abs(null_tstat[indices[1]:indices[2]])) # Summary stat
      ...
      null_tstat_stretch <- null_stretch[, 1]
      null_beta_stretch <- null_stretch[, 2]
      perm_pval <- sum(null_tstat_stretch > tstat_stretch)/niter
      ...
      tstatStr_sd <- sd(unlist(null_tstat_stretch), na.rm = T)
      tstatStr_max <- max(unlist(null_tstat_stretch), na.rm = T)
      tstatStr_mean <- mean(unlist(null_tstat_stretch), na.rm = T)
    ...
    results <- results %>% 
      mutate_cond(electrode == elec, 
                  ...
                  tstat_null_max = tstatStr_max, tstat_null_sd = tstatStr_sd, tstat_null_mean = tstatStr_mean)
