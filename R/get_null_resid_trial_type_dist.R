run_permuted_resid_ttype_null <- function(brain_behave_data, 
                                         electrodes, 
                                         regressors,
                                         regressor_controls,
                                         nBins, 
    ...
    for(reg in regressors){
      ...
      null_df <- foreach(h = 1:niter, .inorder=FALSE, .combine = 'rbind') %dopar% {
        ...
        for (bin in nBins) { # This is the slooooow step
          ...
          ## run models ##
          # get bin and regressor #
          bin_vec_adv <- brain_behave_data_adv %>% pull(bin)
          reg_vec_adv <- brain_behave_data_adv %>% pull(reg)
          control_reg_adv <- brain_behave_data_adv %>% pull(regressor_control)
          
          bin_vec_dis <- brain_behave_data_dis %>% pull(bin)
          reg_vec_dis <- brain_behave_data_dis %>% pull(reg)
          control_reg_dis <- brain_behave_data_dis %>% pull(regressor_control)
          
          # get residuals #
          control_model_adv <- summary(lm(bin_vec_adv ~ control_reg_adv))
          residuals_adv <- control_model_adv$residuals
          
          control_model_dis <- summary(lm(bin_vec_dis ~ control_reg_dis))
          residuals_dis <- control_model_dis$residuals
          
          # run reg on residuals #
          null_lm_adv <-summary(lm(residuals_adv ~ reg_vec_adv))
          null_lm_dis <-summary(lm(residuals_dis ~ reg_vec_dis))
