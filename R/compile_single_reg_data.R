compile_single_reg_data <- function(tag, ofc_file_name, cing_file_name, post_sub_path = "post_eda", pre_tag = NULL, freq = "hfa", beta_permute = F){
  ...
  main_vars <- c("min", "max", "other_var_payoff", "self_var_payoff")
...
### Merge Files Together ###
  ...
  merged_results <- rbind(results_IR57 %>% mutate(subject = "IR57"),
                          ...
                          results_ST40 %>%  select(all_of(colnames(results_IR9))) %>% mutate(subject = "ST40"))
  ...
  write_csv(ofc_merged_results, path(here(), "results", "single_regressions", ofc_file_name))
  write_csv(cing_merged_results, path(here(), "results", "single_regressions", cing_file_name))
